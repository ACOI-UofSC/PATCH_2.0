...
%convert acceleration in bits to Volts
x_a=(x_a/1024)*2.5;
y_a=(y_a/1024)*2.5;
z_a=(z_a/1024)*2.5;

%convert acceleration in V's to G's
x_a=(x_a-1.657)/0.0627;
y_a=(y_a-1.657)/0.0627;
z_a=(z_a-1.657)/0.0627;
...
jerk_actiheart=[diff(mag_actiheart);0]/0.02;
...
[correl, lags]=xcorr(jerk_interp, jerk_actiheart,50*200);
[xx, yy]=max(correl);
...
ENMO=0.02*cumsum(jerk_interp)-movmean(0.02*cumsum(jerk_interp),50); %integrate and remove slow moving artefacts
ENMO_actiheart=0.02*cumsum(jerk_actiheart)-movmean(0.02*cumsum(jerk_actiheart),50);


plot(time_actiheart,ENMO_actiheart, time_actiheart, (1/factor)*ENMO);
figure
scatter(sqrt(movmean(ENMO_actiheart.^2,50*average_time)),(1/factor)*sqrt(movmean(ENMO.^2,50*average_time)),0.2);
hold on

%truncate raw acceleration down to test length
mag_interp=interp1(time,mag,time_actiheart)';
mag_interp=fillmissing(mag_interp,'spline');
mag_actiheart=mag_actiheart(i_start:i_end);
